...
export const Row = styled(FR)<{ inline?: boolean }>`
  ${(props) =>
    props.inline ? 'display: flex; justify-content: space-between;' : ''}
`;
export const Label = styled(L)<{ inline?: boolean }>`
...
export const LabelWrapper = styled.div`
  ...
  width: auto;
`;
export const Input = styled(RadioButtonInput)`
  display: none;
`;
export const InputLabel = styled(Label)<{
  ...
  width: auto;
  border: 1px solid
    ${(props) => (props.checked ? props.theme.primary : props.theme.lightGray)};
  padding: 8px 15px;
  margin: 0px;
